<x-layouts.crm>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-bold text-hando-text-light dark:text-hando-text-dark">Editar Capítulo</h1>
                <p class="mt-1 text-sm text-hando-gray-500 dark:text-hando-gray-400">Modifica la información del capítulo: <strong>{{ $chapter->nombre }}</strong></p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <x-crm.card title="Información del Capítulo">
            <form action="{{ route('admin.chapters.update', [$reportType, $chapter]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <x-hando-input
                            ...
                            :value="old('nombre', $chapter->nombre)"
                    ...
                    <div>
                        ...
                        >{{ old('descripcion', $chapter->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-1 text-sm text-hando-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <x-hando-input
                            ...
                            type="number"
                            class="mt-1 block w-full"
                            :value="old('orden', $chapter->orden)"
                            min="0"
                        />
                        <p class="mt-1 text-xs text-hando-gray-500 dark:text-hando-gray-400">Define el orden en que aparecerá este capítulo.</p>
                        @error('orden')
                            <p class="mt-1 text-sm text-hando-danger">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <div class="mt-6">
            <x-crm.card title="Información de Auditoría">
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        <span class="text-hando-gray-500 dark:text-hando-gray-400">Creado:</span>
                        <span class="ml-2 text-hando-text-light dark:text-hando-text-dark">{{ $chapter->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                    <div>
                        <span class="text-hando-gray-500 dark:text-hando-gray-400">Por:</span>
                        <span class="ml-2 text-hando-text-light dark:text-hando-text-dark">{{ $chapter->creator ? $chapter->creator->name : 'N/A' }}</span>
                    </div>
                    ...
                    <div>
                        ...
                        <span class="ml-2 text-hando-text-light dark:text-hando-text-dark">{{ $chapter->updated_at->format('d/m/Y H:i') }}</span>
                    </div>
                    <div>
                        <span class="text-hando-gray-500 dark:text-hando-gray-400">Por:</span>
                        <span class="ml-2 text-hando-text-light dark:text-hando-text-dark">{{ $chapter->updater ? $chapter->updater->name : 'N/A' }}</span>
                    </div>
                    @endif
                </div>
            </x-crm.card>
