<x-layouts.crm>
    ...
    <div class="max-w-full mx-auto">
        <!-- Indicador del tipo de reporte seleccionado -->
                ...
                <div class="p-5">
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="inline-flex items-center px-4 py-2 rounded-lg text-base font-bold bg-blue-900/90 text-blue-100 shadow-md">
        ...
        <div class="space-y-6">
            ...
            @if($fileGroups->count() > 0)
                @foreach($fileGroups as $grupoId => $files)
                    ...
                    <x-crm.card>
                        ...
                        <!-- Archivos de Entrada Section -->
                        <div class="mb-6">
                            <h4 class="text-sm font-semibold text-hando-gray-700 dark:text-hando-gray-300 mb-3 flex items-center">
                                <svg class="w-4 h-4 mr-2 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                </svg>
                                Archivos de Entrada ({{ $archivosEntrada->count() }})
                            </h4>
                            <div class="space-y-2">
                                @foreach($archivosEntrada as $file)
                                    <div class="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/10 rounded-hando border border-blue-200 dark:border-blue-800">
                                        <div class="flex items-center space-x-3">
                                            <div class="flex-shrink-0 h-10 w-10 bg-blue-200 dark:bg-blue-800 rounded flex items-center justify-center">
                                                <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                                </svg>
                                            </div>
                                            <div>
                                                <p class="text-sm font-medium text-hando-text-light dark:text-hando-text-dark">{{ $file->nombre_original }}</p>
                                                <p class="text-xs text-hando-gray-500">{{ strtoupper($file->extension) }} - {{ $file->tamano_formateado }}</p>
                                            </div>
                                        </div>
                                        <div class="flex items-center space-x-3">
                                            <a href="{{ route('admin.report-files.download', $file) }}" class="text-hando-primary hover:text-hando-primary-hover text-sm">
                                                Descargar
                                            </a>
                                            <form action="{{ route('admin.report-files.destroy', $file) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Eliminar este archivo?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-hando-danger hover:text-red-700 text-sm">
                                                    Eliminar
                                                </button>
                                            </form>
                                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/10 rounded-hando border border-green-200 dark:border-green-800">
                                <div class="flex items-center space-x-3">
                                    <div class="flex-shrink-0 h-10 w-10 bg-green-200 dark:bg-green-800 rounded flex items-center justify-center">
                                        <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                        </svg>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-hando-text-light dark:text-hando-text-dark">{{ $archivoSalida->nombre_original }}</p>
                                        <p class="text-xs text-hando-gray-500">{{ strtoupper($archivoSalida->extension) }} - {{ $archivoSalida->tamano_formateado }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center space-x-3">
                                    <a href="{{ route('admin.report-files.download', $archivoSalida) }}" class="text-hando-primary hover:text-hando-primary-hover text-sm">
                                        Descargar
                                    </a>
                                    <form action="{{ route('admin.report-files.destroy', $archivoSalida) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Eliminar este archivo?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-hando-danger hover:text-red-700 text-sm">
                                            Eliminar
            ...
            <!-- Legacy Files (without grupo_id) -->
            @if($legacyFiles->count() > 0)
                <x-crm.card>
                    <div class="border-b border-hando-border-light dark:border-hando-border-dark pb-4 mb-4">
                        <h3 class="text-lg font-bold text-hando-text-light dark:text-hando-text-dark">
                            Archivos Antiguos (Sin Capítulo)
                        </h3>
                        <p class="text-sm text-hando-gray-500 dark:text-hando-gray-400 mt-1">
                            Archivos subidos antes de la actualización del sistema
                        </p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-hando-gray-50 dark:bg-hando-gray-800">
                                <tr>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-hando-gray-500 dark:text-hando-gray-400 uppercase">Archivo</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-hando-gray-500 dark:text-hando-gray-400 uppercase">Tipo</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-hando-gray-500 dark:text-hando-gray-400 uppercase">Tamaño</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-hando-gray-500 dark:text-hando-gray-400 uppercase">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($legacyFiles as $file)
                                    <tr class="border-t">
                                        <td class="px-4 py-3 text-sm">{{ $file->nombre_original }}</td>
                                        <td class="px-4 py-3 text-sm">{{ strtoupper($file->extension) }}</td>
                                        <td class="px-4 py-3 text-sm">{{ $file->tamano_formateado }}</td>
                                        <td class="px-4 py-3 text-sm space-x-2">
                                            <a href="{{ route('admin.report-files.download', $file) }}" class="text-hando-primary hover:text-hando-primary-hover">
                                                Descargar
                                            </a>
                                            <form action="{{ route('admin.report-files.destroy', $file) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Eliminar?');">
                                                @csrf
                                                @method('DELETE')
            ...
            <!-- Empty State -->
            @if($fileGroups->count() === 0 && $legacyFiles->count() === 0)
                <x-crm.card>
